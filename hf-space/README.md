...
- **Qwen3.5-4B (q4f16_1)** — append `?model=qwen35` to `zero-tvm.html` or `validate.html`; weights stream from [`mlc-ai/Qwen3.5-4B-q4f16_1-MLC`](https://huggingface.co/mlc-ai/Qwen3.5-4B-q4f16_1-MLC) (~2.6 GB). The first *hybrid* on the engine: 24 gated-DeltaNet (linear-attention) layers + 8 gated-attention layers (GQA 16/4, head_dim 256, partial RoPE, sigmoid attention gate) — to our knowledge the first hand-written-kernel int4 gated-DeltaNet hybrid in a browser. Measured 2026-07-28 (Apple M2 Max, same session, identical weight bytes): Zero-TVM **47.99 tok/s** vs WebLLM 0.2.84's prebuilt Qwen3.5-4B at **31.99 tok/s** (+50%) — on a v1 scalar-GDN path (no chunked prefill, unfused GDN projections), so the number is a floor and the gap is one pair on one machine; caveats in [BENCH.md](https://github.com/abgnydn/zero-tvm/blob/main/BENCH.md).
