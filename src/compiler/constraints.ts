...
export interface DetectedModel {
  ...
  chatTemplate: 'chatml' | 'chatml-q35' | 'chatml-q38' | 'phi3' | 'llama3' | 'unknown'
}
...
export const SUPPORT_MATRIX: MatrixRow[] = [
  ...
  { area: 'Attention', supported: 'MHA and GQA, headDim %32 ≤256, full RoPE or partial (rotary fraction), qk-norm optional, gated attention (Qwen3.5/3.6), paged f16 KV; int8 KV on any headDim (packing strides, so 256 is covered)', not: 'sliding window, MLA, ALiBi, softcap, attention biases', needs: 'windowed/MLA variants of attention.wgsl; bias epilogue in matmuls' },
 ...
 */
/**
 * WHICH ChatML — one family, three renderers.
 *
 * Returning a bare 'chatml' for a Qwen3.5-or-later checkpoint is not cosmetic.
 * It picks the wrong past-turn <think> rule AND, through toolDialectFor, the
 * wrong tool-call dialect: the model is handed a tools block it cannot follow,
 * which reads as "this model is bad at tools" rather than as a wiring bug.
 *
 * This is not hypothetical. QWEN3_8_27B_4BIT was generated by add-model on
 * 2026-08-17 with 'chatml' and hand-corrected two days later.
 *
 * Keyed on the BEHAVIOUR each template encodes, not on a version string in a
 * name, because the name is not in the file:
 *   - `preserve_thinking is undefined` always fires and is Qwen3.8's rule
 *     (Qwen3.6 names the same variable behind `is defined`, which never fires);
 *   - the `loop.last or (not loop.last and reasoning_content)` gate is the
 *     Qwen3-era extra condition;
 *   - `ns.last_query_index` without that gate is Qwen3.5/3.6.
 * A ChatML template with none of these markers is some other vendor's and stays
 * plain 'chatml' — the pre-2026-08 behaviour, unchanged.
 */
function chatmlGeneration(t: string): string {
  // The CLAUSE, not the identifier. Qwen3.6 also mentions preserve_thinking,
  // guarded as `is defined and ... is true` — which never fires, because
  // nothing defines it. Qwen3.8 opens with `is undefined or`, which always
  // does. Matching the bare name maps 3.6 onto 3.8's rule.
  if (t.includes('preserve_thinking is undefined')) return 'chatml-q38'
  if (t.includes('loop.last or (not loop.last and reasoning_content)')) return 'chatml'
  if (t.includes('ns.last_query_index')) return 'chatml-q35'
  return 'chatml'
}

export function detectChatTemplate(templateText: string, bosTokenStr: string | null): string {
  return templateText.includes('<|im_start|>') ? chatmlGeneration(templateText)
