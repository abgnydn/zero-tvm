...
const EXPERT_PATH =
	/(switch_mlp|shared_expert)\.(gate_proj|up_proj|down_proj)$/;
...
export function checkModel(m: DetectedModel): CheckResult {
	...
	const notes: string[] = [];
	const fail = (rule: string, detail: string, needs: string) =>
		failures.push({ rule, detail, needs });

	// ── container ──────────────────────────────────────────────
	if (!m.hasIndex) {
		fail(
			"index",
			"model.safetensors.index.json is missing from the repo",
	...
	if (!m.quant) {
		fail(
			"quant",
			"checkpoint is not quantized (f16/bf16 weights)",
		...
		if (m.quant.bits !== 4 || m.quant.groupSize !== 64) {
			fail(
				"quant",
				`base quantization is ${m.quant.bits}-bit group ${m.quant.groupSize}`,
		...
		for (const [path, q] of Object.entries(m.quant.overrides)) {
			...
			const routerOk =
				ROUTER_PATH.test(path) &&
				((q.bits === 8 && q.groupSize === 64) || q.bits === 16);
			const expertOk =
				EXPERT_PATH.test(path) &&
				(q.bits === 3 || q.bits === 4) &&
				q.groupSize === 64;
			if (!routerOk && !expertOk && !(q.bits === 4 && q.groupSize === 64)) {
				fail(
					"quant",
					`'${path}' is ${q.bits}-bit group ${q.groupSize}`,
	...
	if (
		m.ropeScalingType !== null &&
		m.ropeScalingType !== "llama3" &&
		m.ropeScalingType !== "yarn"
	) {
		fail(
			"rope",
			`rope_scaling '${m.ropeScalingType}' (longrope-style frequency remapping)`,
	...
	if (m.ropeScalingType === "yarn") {
		...
		const missing =
			!y ||
			[y.factor, y.betaFast, y.betaSlow, y.originalMaxPositionEmbeddings].some(
				(v) => v === null || v === undefined,
			);
		if (missing) {
			fail(
				"rope",
				"rope_scaling is yarn but the config omits factor / beta_fast / beta_slow / original_max_position_embeddings",
	...
	if (m.ropeScalingType === "llama3") {
		...
		const missing =
			...
			[
				...
				rs.lowFreqFactor,
				rs.highFreqFactor,
				rs.originalMaxPositionEmbeddings,
			].some((v) => typeof v !== "number");
		if (missing) {
			fail(
				"rope",
				"rope_scaling llama3 without factor/low_freq_factor/high_freq_factor/original_max_position_embeddings",
	...
	if (m.mlpBias) {
		fail(
			"bias",
			"FFN biases (mlp_bias)",
			"the matmul kernels have no additive-bias epilogue",
		);
	}
	if (m.attnBias) {
		...
		fail(
			"bias",
			`linear-layer biases (attention_bias / *.bias records)${eg}`,
	...
	if (m.layerTypes) {
		...
		for (const t of new Set(m.layerTypes.filter((t) => !known.has(t)))) {
			fail(
				"blocks",
				`layer_types contains unknown block type '${t}'`,
	...
	if (missing.length) {
		fail(
			"config",
			`could not resolve ${missing.join(", ")} from config.json (family '${m.family || "?"}')`,
		...
		if (heads % kvHeads !== 0)
			fail(
				"dims",
				`heads ${heads} % kvHeads ${kvHeads} != 0`,
				"GQA group must be integral (attention.wgsl maps q-heads to kv-heads by division)",
			);
		if (headDim % 32 !== 0)
			fail(
				"dims",
				`headDim ${headDim} % 32 != 0`,
		...
		if (headDim > 256)
			fail(
				"dims",
				`headDim ${headDim} > 256`,
				"per-head shared-memory sizing in attention.wgsl",
			);
		if (d % 256 !== 0)
			fail(
				"dims",
				`d ${d} % 256 != 0`,
				"add_norm/embedding grids are d/256 workgroups with no tail",
			);
		if (qkvDim % 256 !== 0)
			fail(
				"dims",
				`qkvDim ${qkvDim} % 256 != 0`,
				"rope grid is qkvDim/256 workgroups with no tail",
			);
		if (kvDim % 256 !== 0)
			fail(
				"dims",
				`kvDim ${kvDim} % 256 != 0`,
				"kv_append grid is kvDim/256 workgroups with no tail",
			);
		...
		if (vocab % 4 !== 0 && !m.embeddingOnly) {
			fail(
				"dims",
				`vocab ${vocab} % 4 != 0`,
				"lm_head runs rowsPerWG=4 — the remainder rows would silently drop",
			);
		}
		...
		for (const [name, k] of ks) {
			if (k % 64 !== 0)
				fail(
					"dims",
					`matmul K=${name} (${k}) % 64 != 0`,
					"affine scale groups are 64 wide",
				);
		}
	}
	...
	if (m.unknownConfigKeys?.length) {
		fail(
			"config",
			`config.json carries ${m.unknownConfigKeys.length} field(s) this checker does not read: ${m.unknownConfigKeys.join(", ")}`,
	...
	if (m.mla) {
		...
		const mha =
			m.kvHeads !== null && m.headDim !== null ? m.kvHeads * m.headDim * 2 : 0;
		fail(
			"attention",
			`multi-head latent attention (kv_lora_rank ${m.mla.kvLoraRank}` +
	...
	if (m.moe) {
		...
		if (m.moe.denseLayers.length) {
			fail(
				"moe",
				`layers [${m.moe.denseLayers.join(", ")}] run a dense FFN while the rest are MoE`,
		...
		if (m.moe.scoringFunc && m.moe.scoringFunc !== "softmax") {
			fail(
				"moe",
				`router scoring '${m.moe.scoringFunc}' (not softmax)`,
		...
		if (m.moe.sparseStep !== 1) {
			fail(
				"moe",
				`only every ${m.moe.sparseStep}th layer is MoE (decoder_sparse_step)`,
		...
		if (
			m.moe.sharedExpert &&
			m.moe.sharedFfn !== null &&
			m.ffn !== null &&
			m.moe.sharedFfn !== m.ffn
		) {
			fail(
				"moe",
				`shared expert width ${m.moe.sharedFfn} != moe_intermediate ${m.ffn}`,
		...
		if (m.moe.topK > 32)
			fail(
				"moe",
				`top-${m.moe.topK} > 32`,
				"one router slot per lane in moe_router_topk.wgsl",
			);
		notes.push(
			"MoE requires WebGPU subgroups — there is no scalar router/expert path",
		);
	}

	// ── GDN (linear attention) ─────────────────────────────────
	if (m.gdn) {
		if (m.gdn.vHeads % m.gdn.kHeads !== 0)
			fail(
				"gdn",
				`vHeads ${m.gdn.vHeads} % kHeads ${m.gdn.kHeads} != 0`,
				"GVA grouping in gdn_recur.wgsl",
			);
		if (m.gdn.headV % 32 !== 0 || m.gdn.headV > 256)
			fail(
				"gdn",
				`headV ${m.gdn.headV}`,
				"gdn_recur/gdn_norm_out workgroup sizing (32-lane rows, ≤256)",
			);
		if (m.gdn.convK !== 4)
			fail(
				"gdn",
				`conv kernel width ${m.gdn.convK} != 4`,
				"gdn_conv.wgsl ring state is sized for width 4",
			);
		...
		if (m.gdnOutputGate != null && m.gdnOutputGate !== "swish") {
			fail(
				"gdn-gate",
				`output_gate_type '${m.gdnOutputGate}' — gdn_norm_out applies silu (= swish)`,
				`a gdn_norm_out variant applying ${m.gdnOutputGate}(z)`,
			);
		}
		notes.push(
			"GDN layers force the unfused composition (no fused QKV); int8 KV (?kv8=1) IS supported",
		);
	}
	...
	if (m.tokenizer === "unknown") {
		fail(
			"tokenizer",
			"tokenizer.json is neither SentencePiece nor byte-level BPE",
	...
	if (m.chatTemplate === "unknown") {
		fail(
			"template",
			"chat template is none of Phi-3, ChatML, Llama-3 or DeepSeek",
	...
	if (m.quant)
		notes.push(
			"MLX-affine runs the unfused composition (no fused QKV, 3-dispatch dense FFN); chunked prefill is ON (affine batched GEMM, E5)",
		);
...
export const SUPPORT_MATRIX: MatrixRow[] = [
	{
		area: "Weight format",
		supported:
			"MLC q4f16_1 shards (group 32, symmetric); MLX safetensors (group 64, affine, 4-bit; 8-bit router; 3-bit expert stacks via convert-q3-experts)",
		not: "f16/bf16 unquantised, GPTQ/AWQ, other group sizes",
		needs: "new dequant paths in int4_matmul.gen.ts",
	},
	{
		area: "Attention",
		supported:
			"MHA and GQA, headDim %32 ≤256, full RoPE or partial (rotary fraction), qk-norm optional, gated attention (Qwen3.5/3.6), paged f16 KV; int8 KV on any headDim (packing strides, so 256 is covered)",
		not: "sliding window, MLA, ALiBi, softcap, attention biases",
		needs: "windowed/MLA variants of attention.wgsl; bias epilogue in matmuls",
	},
	{
		area: "RoPE",
		supported:
			"plain theta (any base), partial rotary factor, llama3 and yarn frequency scaling (precomputed inv_freq table; yarn also scales attention logits by mscale^2)",
		not: "longrope frequency scaling",
		needs:
			"a frequency formula in model-spec.ts ropeInvFreqTable() — rope.wgsl already reads the table",
	},
	{
		area: "FFN",
		supported:
			"SwiGLU dense (fused kernel for MLC, matmul+silu_mul chain for MLX-affine)",
		not: "GeGLU / ReLU / non-gated FFN, FFN biases",
		needs: "activation-parameterised fused_ffn.wgsl + silu_mul.wgsl",
	},
	{
		area: "Norm",
		supported: "RMSNorm with plain gamma",
		not: "LayerNorm (beta), Gemma-style (1+gamma), post-norm sandwiches",
		needs: "variants of rms_norm/add_norm.wgsl",
	},
	{
		area: "MoE",
		supported:
			"≤256 routed experts, top-K ≤32, with OR without a shared expert (when present it stacks as index E and must equal moe_intermediate in width), router at 4-bit, 8-bit or unquantized f16, norm_topk_prob either way; subgroups required",
		not: "grouped/expert-parallel routing, a stack mixing dense and MoE layers",
		needs:
			"a routing layout where experts are sharded across dispatches rather than stacked; a per-layer block kind for mixed stacks",
	},
	{
		area: "Linear attention",
		supported: "GatedDeltaNet (Qwen3.5/3.6): GVA, headV %32 ≤256, conv width 4",
		not: "Mamba/S4, RWKV, conv hybrids (LFM2 layer_types 'conv'), other conv widths",
		needs: "new recurrence kernels",
	},
	{
		area: "Embedding / head",
		supported:
			"quantised embedding (symmetric or affine), tied or untied lm_head, vocab %4",
		not: "unquantised embedding tables",
		needs: "f16 gather path",
	},
	{
		area: "Tokenizer",
		supported:
			"SentencePiece (Phi-3), byte-level BPE (Qwen/Llama-style tokenizer.json)",
		not: "tekken, WordPiece, custom pipelines",
		needs: "new pipeline beside tokenizer-bpe.ts",
	},
	{
		area: "Chat template",
		supported:
			"Phi-3, ChatML (non-thinking), Llama-3 header template, DeepSeek prose turns",
		not: "Gemma turns, Mistral [INST], thinking-mode rendering",
		needs:
			"renderer branch in model-select.ts — the single highest-leverage gap in the survey (docs/PORTING.md): it blocks 29 of 51 refused repos and is the SOLE blocker on 5",
	},
	{
		area: "Decoding",
		supported:
			"greedy argmax (default), seeded temperature / top-p / min-p sampling, streaming, cross-turn prefix reuse",
		not: "top-k, repetition/presence penalties, beam search, batch > 1",
		needs:
			"a rank selection pass beside sampler.wgsl's mass threshold; a per-sequence token-count buffer for penalties",
	},
];
...
export function detectChatTemplate(
	templateText: string,
	bosTokenStr: string | null,
): string {
	return templateText.includes("<|im_start|>")
		? "chatml"
		: templateText.includes("<|start_header_id|>")
			? "llama3"
			: templateText.includes("<|user|>") && templateText.includes("<|end|>")
				? "phi3"
				: templateText.includes("'User: '") &&
						templateText.includes("'Assistant: '")
					? "deepseek"
					: // [INST] family. All three are '[INST]' templates and NONE of them says
						...
						? templateText.includes("not tools is defined")
							? "ministral"
							: templateText.includes("After the optional system message")
								? "mistral-nemo"
								: templateText.includes("Conversation roles must alternate")
									? "mistral"
									: "unknown"
