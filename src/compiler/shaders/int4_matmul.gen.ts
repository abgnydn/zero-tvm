...
export function int4MatmulEntry(opts: Int4MatmulOpts = {}): string {
  const { outF32 = false, rowsPerWG = 1, subgroups = false, m = 1, vec4 = false, mDyn = false } = opts
...
export function int4MatmulWGSL(opts: Int4MatmulOpts = {}): string {
  ...
  if (mDyn) {
    if (!subgroups || rowsPerWG !== 4 || m !== 1 || vec4 || outF32)
...
// Variant: ${entry} (out=${outType}, rowsPerWG=${rowsPerWG}, ${subgroups ? 'subgroupAdd' : 'tree reduction'}${m > 1 ? `, M=${m}` : ''}${vec4 ? ', vec4 loads' : ''})
...
@group(0) @binding(1) var<storage, read> input_buf : array<${vec4 ? 'vec4<u32>' : 'f16'}>;
@group(0) @binding(2) var<storage, read> scales : array<f16>;
@group(0) @binding(3) var<storage, read> weights : array<${vec4 ? 'vec4<u32>' : 'u32'}>;
