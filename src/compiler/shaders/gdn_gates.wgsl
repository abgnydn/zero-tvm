...
//
// Grid: seq_len workgroups (one per token; podArgs.packGridDimX = seq_len),
// 32 threads looping v-heads. Decode dispatches seq_len = 1; chunked prefill
// binds the BATCHED fused-projection buffer at the first token's [a|b] offset
// and strides tokens by ab_stride (= gdnProjRows; GDN_V_HEADS*2 when the
// buffer is tightly packed, e.g. decode's single-token region view).
// Model-shape constants are injected by src/compiler/shader-prelude.ts.

enable f16;

@group(0) @binding(0) var<storage, read_write> gates : array<f32>;  // seq * 2*GDN_V_HEADS ([exp(g) | beta] per token)
@group(0) @binding(1) var<storage, read> ab_raw : array<f16>;       // seq * ab_stride ([a | b] head)
...
struct PODArgs {
  seq_len: i32,
  ab_stride: i32,     // f16 elements between consecutive tokens' [a|b] pairs
  packGridDimX: u32
}
...
fn gdn_gates(
  ...
  let t : i32 = wg;                      // token index within the chunk
  if (t >= podArgs.seq_len) { return; }
  let ab : i32 = t * podArgs.ab_stride;
  let out : i32 = t * 2 * GDN_V_HEADS;

  for (var h : i32 = i32(threadIdx.x); h < GDN_V_HEADS; h = h + 32) {
    let x : f32 = f32(ab_raw[ab + h]) + dt_bias[h];
    let sp : f32 = select(log(1.0 + exp(x)), x, x > 20.0);  // softplus, MLC threshold
    gates[out + h] = exp(-exp(a_log[h]) * sp);
    gates[out + GDN_V_HEADS + h] = 1.0 / (1.0 + exp(-f32(ab_raw[ab + GDN_V_HEADS + h])));
  }
}
