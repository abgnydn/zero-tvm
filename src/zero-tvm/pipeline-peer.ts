/**
 * PIPELINE PEER — the wire between two halves of one model.
 *
 * Stage 0 holds layers [0, k) and drives the token loop; the helper holds
 * [k, layers) and answers. Per token, exactly one round trip:
 *
 *   stage 0 → [u32 reqId][u32 position][residual bytes]   (d f16: 4-5 KB)
 *   helper  → {"type":"stage-token","req":n,"tokenId":t}  (~40 B)
 *
 * That asymmetry is the whole latency story. Bandwidth is trivial — 5 KB per
...
/**
 * HELPER side: answer residual frames with tokens, forever.
 *
  ...
  dc.addEventListener('message', (e) => {
    ...
    queue = queue.then(async () => {
      ...
      try {
        ...
        if (!('tokenId' in out)) throw new Error('helper stage did not end the model')
        dc.send(JSON.stringify({ type: 'stage-token', req: reqId, tokenId: out.tokenId }))
...
/**
 * STAGE-0 side: hand a residual over and wait for the token it becomes.
 *
...
export function makeStageClient(dc: RTCDataChannel): {
  step: (position: number, residual: ArrayBuffer) => Promise<number>
  /** Mean round-trip over the steps so far, in ms — the split's real cost. */
  meanHopMs: () => number
} {
  dc.binaryType = 'arraybuffer'
  const waiting = new Map<number, { resolve: (t: number) => void; reject: (e: Error) => void; t0: number }>()
  let nextId = 1
  let hops = 0
  let hopMs = 0
  ...
  dc.addEventListener('message', (e) => {
    ...
    if (msg.type === 'stage-token' && typeof msg.tokenId === 'number') {
      ...
      w.resolve(msg.tokenId)
    } else {
      w.reject(new Error(msg.message ?? 'the other stage failed'))
    }
  })
  dc.addEventListener('close', () => {
    for (const w of waiting.values()) w.reject(new Error('the other half of the model disconnected'))
    waiting.clear()
  })

  return {
    step(position, residual) {
      if (dc.readyState !== 'open') return Promise.reject(new Error('the other half of the model is not connected'))
      const reqId = nextId++
