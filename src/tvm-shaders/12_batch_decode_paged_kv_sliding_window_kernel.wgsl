// [12] batch_decode_paged_kv_sliding_window_kernel (shader #12, 199 lines)
//----------------------------------------
  ...
  for (var iterator : i32 = 0; iterator < (((kv_chunk_len[0i] - 2147483621i) / 20i) - -107374182i); iterator++) {
    ...
    for (var j : i32 = 0; j < 2i; j++) {
      if ((((iterator * 20i) + (i32(threadIdx.z) * 2i)) + j) < kv_chunk_len[0i]) {
                var condval_3 : i32;
        if (((((iterator * 20i) + (i32(threadIdx.z) * 2i)) + j) < length_info[(((podArgs.B * 2i) + v__1) + podArgs.length_info_elem_offset)])) {
          condval_3 = (((iterator * 20i) + (i32(threadIdx.z) * 2i)) + j);
} else {
          condval_3 = (((((iterator * 20i) + (i32(threadIdx.z) * 2i)) + j) + length_info[((v__1 + podArgs.B) + podArgs.length_info_elem_offset)]) - length_info[(((podArgs.B * 2i) + v__1) + podArgs.length_info_elem_offset)]);
}
let seq_offset : i32 = condval_3;
        ...
        if ((podArgs.rotary_mode == 1i)) {
          ...
          if ((i32(threadIdx.x) < 12i)) {
            let v__2 : i32 = ((((((page_no * 98304i) + (i32(blockIdx.y) * 1536i)) + ((seq_offset & 15i) * 96i)) + (i32(threadIdx.x) * 4i)) + podArgs.pages_elem_offset) + 48i);
            condval_5 = (vec4<f16>(pages[v__2 + 0], pages[v__2 + 1], pages[v__2 + 2], pages[v__2 + 3]) * vec4<f16>(-1.000000e+00h, -1.000000e+00h, -1.000000e+00h, -1.000000e+00h));
} else {
            let v__3 : i32 = ((((((page_no * 98304i) + (i32(blockIdx.y) * 1536i)) + ((seq_offset & 15i) * 96i)) + (i32(threadIdx.x) * 4i)) + podArgs.pages_elem_offset) - 48i);
            condval_5 = vec4<f16>(pages[v__3 + 0], pages[v__3 + 1], pages[v__3 + 2], pages[v__3 + 3]);
}
          let v__4 : i32 = (((((page_no * 98304i) + (i32(blockIdx.y) * 1536i)) + ((seq_offset & 15i) * 96i)) + (i32(threadIdx.x) * 4i)) + podArgs.pages_elem_offset);
          condval_4 = vec4<f16>(fma(sin(freq_1), vec4<f32>(condval_5), (cos(freq_1) * vec4<f32>(vec4<f16>(pages[v__4 + 0], pages[v__4 + 1], pages[v__4 + 2], pages[v__4 + 3])))));
} else {
          let v__5 : i32 = (((((page_no * 98304i) + (i32(blockIdx.y) * 1536i)) + ((seq_offset & 15i) * 96i)) + (i32(threadIdx.x) * 4i)) + podArgs.pages_elem_offset);
          condval_4 = vec4<f16>(pages[v__5 + 0], pages[v__5 + 1], pages[v__5 + 2], pages[v__5 + 3]);
}
        K_smem[(((i32(threadIdx.z) * 48i) + (j * 24i)) + i32(threadIdx.x))] = condval_4;
        let v__6 : i32 = ((((((page_no * 98304i) + (i32(blockIdx.y) * 1536i)) + ((seq_offset & 15i) * 96i)) + (i32(threadIdx.x) * 4i)) + podArgs.pages_elem_offset) + 49152i);
