...
1. ~~**adapter limits**~~ **MEASURED 2026-08-10**: both 4096 MiB on this M2 Max (BENCH.md, KV paging feasibility). A 20k Qwen3.5 prefix is ~78 MiB per attention layer, so Phase 3 does NOT need split per-layer bindings.
2. ~~**OPFS throughput**~~ **MEASURED 2026-08-10**: ~7.7 GB/s write, ~8 GB/s read at 625 MiB — but through the page cache, so an upper bound. Not the bottleneck; `writeBuffer` is.
3. ~~**GPU→CPU readback**~~ **MEASURED 2026-08-10**: 3.2-8.9 GB/s, best at 64 MiB chunks (16 MiB costs ~30%). Plus the number the plan omitted — CPU→GPU `writeBuffer`, 2.4-6.7 GB/s, the slowest link in the chain. Save 0.29 s, restore 0.36 s for 625 MiB at the slowest run: 56x under the 20 s threshold.
